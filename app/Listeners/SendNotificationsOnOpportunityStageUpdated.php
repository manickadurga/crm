...
class SendNotificationsOnOpportunityStageUpdated
{
    private $context = [];
    /**
     * Handle the event.
     *
     * @param \App\Events\OpportunityStageUpdated $event
     * @return void
     */
    public function handle(OpportunityStageUpdated $event)
{
    Log::info("OpportunityStageUpdated event triggered.");
    $opportunity = $event->opportunity;
    Log::info("Processing opportunity:", ['id' => $opportunity->id, 'name' => $opportunity->opportunity_name]);

    // Fetch workflows associated with the 'opportunity_stage_updated' trigger
    $workflows = Workflow::whereHas('trigger', function ($query) {
        $query->where('trigger_name', 'opportunity_stage_updated');
    })->get();

    if ($workflows->isEmpty()) {
        Log::info("No workflows found for 'opportunity_stage_updated' trigger.");
    }
...
private function findContact(array $actionData, $opportunity)
    ...
    // Replace placeholders in the message with opportunity data
    $message = str_replace(
        ['{{opportunity.name}}', '{{opportunity.stage}}'],
        [$opportunity->opportunity_name, $opportunity->select_stage],
        $data['message']
    );
    ...
    try {
        ...
        Log::info("SMS sent to {$phoneNumber}: {$sentMessage->sid}");
    } catch (\Exception $e) {
    ...
    {
        $contact = Customers::find($opportunity->contact_id);
        $email = $contact ? $contact->primary_email : null;

        if ($email) {
            $subject = str_replace('{{opportunity.name}}', $opportunity->opportunity_name, $data['subject']);
            $message = str_replace(
                ['{{opportunity.name}}', '{{opportunity.stage}}'],
                [$opportunity->opportunity_name, $opportunity->select_stage],
                $data['message']
            ...
            Log::warning("No email found for opportunity contact ID: {$opportunity->contact_id}");
        }
    }
    ...
    private function sendWhatsApp(array $data, $opportunity)
{
    ...
    if (!$whatsappNumber) {
        Log::error("No WhatsApp number found for contact_id: {$opportunity->contact_id}");
    ...
    // Log WhatsApp details
    Log::info("Sending WhatsApp message to: {$whatsappNumber}");
    Log::info("Message: {$message}");

    // Check for missing Twilio credentials
    if (empty($sid) || empty($token) || empty($twilioWhatsappNumber)) {
        Log::error("Twilio credentials are missing. SID: {$sid}, Token: {$token}, WhatsApp Number: {$twilioWhatsappNumber}");
        return;
    }
    ...
    try {
        ...
        $sentMessage = $client->messages->create(
            "whatsapp:{$whatsappNumber}", // To (formatted for WhatsApp)
            [
        ...
        Log::info("WhatsApp message sent to {$whatsappNumber}: {$sentMessage->sid}");
    } catch (\Exception $e) {
