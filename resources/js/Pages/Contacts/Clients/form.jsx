import {
  ...
  const ClientForm = () => {
    ...
    // Fetch form fields
    useEffect(() => {
        getFormfields('Clients')
            .then((res) => {
                setClientFormFields(res || []);
            })
            .catch((error) => {
    ...
    useEffect(() => {
        console.log("clientFormFields state has been set:", clientFormFields);
    }, [clientFormFields]);
    ...
    // Set form values when client data is fetched
    useEffect(() => {
        if (client && id) {
    ...
    const handleSelectChange = (value, field) => {
      ...
      clientFormFields.forEach(section => {
    ...
    const updateClientData = async (id, data) => {
      try {
        const response = await axios.put(`http://127.0.0.1:8000/clients/${id}`, data);
        return response.data;
      } catch (error) {
    ...
    const onFinish = async (values) => {
      ...
      try {
        if (id) {
          await updateClientData(id, formDataValues);
        } else {
          await storeClientData(formDataValues);
        }
        message.success(id ? 'Client Details Updated Successfully' : 'Client Details Added Successfully');
        // const customers = await getCustomers();
        // setFormCustomers(customers);
        navigate('/clients'); // Navigate to the customer page
      } catch (error) {
    ...
    const steps = clientFormFields.map((section, sectionIndex) => ({
                      ...
                      <div>
                        ...
                        <LeafletMap
                          ...
                          defaultValues={{
                            ...
                            postcode: client.location.zipcode
                          }}
                        />
    ...
    const next = async () => {
      try {
        ...
        clientFormFields[current].fields.forEach(field => {
        ...
        const mandatoryFields = clientFormFields[current].fields.filter(field => field.rules && field.rules.some(rule => rule.required));
